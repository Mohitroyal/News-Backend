...
import { useAuthStore, saveReporterPhoto } from '@/store';
...
export const SignupScreen = () => {
  ...
  useEffect(() => {
    ...
    if (passedAvatar && !avatarUrl) {
      setAvatarUrl(passedAvatar);
    }
  }, [passedEmail, passedAvatar]);
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          if (email) saveReporterPhoto(email, dataUrl);
        } else {
    ...
    try {
      ...
      if (response && response.authentication) {
        ...
        if (sessionData.session) {
          ...
          const finalPhoto = avatarUrl || userObj?.user_metadata?.avatar_url || userObj?.user_metadata?.picture;
          if (finalPhoto) {
            userObj.avatarUrl = finalPhoto;
            if (googleEmail) saveReporterPhoto(googleEmail, finalPhoto);
          }
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 relative z-10">
        <div className="w-full max-w-md bg-white border border-[#b8d4e8] rounded-xl p-8 shadow-sm">
          ...
          <div className="mb-6 flex flex-col items-start">
            ...
            <h1 className="text-[#0a1a2e] text-2xl font-bold font-serif mb-2">Create New Account</h1>
            ...
            >
              {avatarUrl ? (
                ...
                <div className="flex flex-col items-center justify-center text-[#0a2540]">
                  <User className="w-8 h-8 text-[#0a2540]/60 mb-0.5" />
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div className="relative">
              <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#a0c4dc]" />
              <input
                type="text"
                placeholder="Full Name / Journalist Name"
                value={name}
          ...
          <div className="mt-8 text-center flex flex-col gap-2">
            <p className="text-[#a0c4dc] text-xs font-medium">
              ...
              <Link to="/login" className="text-[#0a2540] font-bold hover:underline transition-colors">
                Sign In
              </Link>
            </p>
          </div>
