...
export const GenerateScreen = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const totalImages = imageUrls.length > 0 ? imageUrls.length : (currentConfig.imageUrl ? 1 : 0);
      let derivedImageLayout: string = currentConfig.imageLayout || '';
      if (!derivedImageLayout || derivedImageLayout === 'default' || derivedImageLayout === 'auto') {
        if (totalImages <= 1) {
          derivedImageLayout = 'pattern_b';
        } else {
          derivedImageLayout = currentConfig.templateId?.endsWith('_b') ? 'pattern_b' : 'pattern_a';
        }
      }
