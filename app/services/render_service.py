...
{json_str}
        ...
        document.addEventListener("DOMContentLoaded", async () => {
            ...
            async function executeLayout() {
                ...
                const estFontSize = Math.sqrt(Math.max(100000, N * W_col * H_avail - obstacleArea) / (totalChars * 0.42));
                const conf = { fontSize: Math.max(16.0, Math.min(28.0, estFontSize)), lineHeight: 1.35, paraMargin: 12, imgMaxPct: 0.58, padding: 32 };
